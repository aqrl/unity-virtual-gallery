...
namespace Invector.vCharacterController.vActions
{
    ...
    public class vGenericAction : vActionListener
    {
        ...
        [Tooltip("Layer of the object you want to access")]
        ...
        protected override void Start()
        {
            ...
            vFPCameraUi = FindObjectOfType<vFPCameraUI>();
        }
        ...
        bool enterAction;
        vTriggerGenericAction PreviousAction = null;
        private void Update()
        {
            vTriggerGenericAction MouseOverAction = IsMouseOverAction();

            if (MouseOverAction != null)
            {
                PreviousAction = MouseOverAction;

                if(!enterAction)
                    OnActionEnter(MouseOverAction._collider);

                enterAction = true;

                OnActionStay(MouseOverAction._collider);
                return;
            }
            enterAction = false;
            ...
            {
                OnActionExit(PreviousAction._collider);
                PreviousAction = null;
            }
        }
        private vTriggerGenericAction IsMouseOverAction()
        {
            RaycastHit objectHit;

            if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out objectHit, 1.5f))
            {
                if (objectHit.transform.TryGetComponent(out vTriggerGenericAction triggerAction))
                {
                    if(triggerAction.mouseOver)
        ...
        /*private bool IsMouseOverObject(GameObject ActionObject)
        {
            RaycastHit objectHit;

            if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out objectHit, 1f, triggerLayer))
            {
                GameObject triggerAction = objectHit.collider.gameObject;

                if(triggerAction.transform.parent != null)
                {
                    GameObject actionParent = triggerAction.transform.parent.gameObject;
                    if (actionParent.TryGetComponent(out vTriggerGenericAction hasAction) && actionParent == ActionObject)
                    {
                        return true;
                    }
                ...
                if(triggerAction.TryGetComponent(out vTriggerGenericAction isActionTrigger))
                {
                    if(triggerAction == ActionObject)
                        return true;
                }
            }
        ...
        public void DisplayStats(vTriggerGenericAction triggerAction)
        {
            if(triggerAction.TryGetComponent(out Art_Details artDetails))
            {
                vFPCameraUi.ArtDisplay(artDetails);
            }
        }
